...
Otherwise issue "GREEN". A submitter's own state, city, language, and the family member a recipe belongs to are expected context — every entry carries them — and are never grounds for RED. Family memories are not claims to fact-check; do not judge authenticity, only the list above.
