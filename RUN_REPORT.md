...
| `npm run localize:corpus` | localize-corpus.ts | **PASS** | Precomputes the localized restoration cards: 31 searchable records × 7 non-English languages = 217 pairs, ~$0.83 one-time, content-hash idempotent. Writes `corpus/localized/<lang>/<slug>.json` (committed, reviewable). Live (spends API); not in `check`. Re-run only re-translates changed records. |
