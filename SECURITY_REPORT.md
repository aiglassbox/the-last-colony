...
1. **`KITCHEN_SECRET` fallback** (`src/lib/dash/auth.ts:40`): the HMAC signing key defaults to `kitchen:${password}`, so session-token forgery reduces exactly to password entropy. Set `KITCHEN_SECRET` to 32+ random bytes in production to decouple them. *(Effort: env var only.)*
